...
namespace SmoothCoastlines.Rivers {
    ...
    public class GenTerraPrety : ModStdWorldGen { //This is a copied over version of GenTerra from Vanilla on 8/27/2025. Will need to keep this up to date and keep parity with vanilla to account for changes.
        ...
        public override void AssetsFinalize(ICoreAPI coreApi) {
            ...
            TerraGenConfig.seaLevel = (int)(0.4313725490196078 * (api.WorldManager.MapSizeY - 64));
            api.WorldManager.SetSeaLevel(TerraGenConfig.seaLevel);
            Climate.Sealevel = TerraGenConfig.seaLevel;
        }

        public override void StartServerSide(ICoreServerAPI api) {
            var initWorldGenHandlers = ((ServerMain)api.World).ModEventManager.WorldgenHandlers["standard"].OnInitWorldGen;
            var onChunkColumnGenHandlers = ((ServerMain)api.World).ModEventManager.WorldgenHandlers["standard"].OnChunkColumnGen[(int)EnumWorldGenPass.Terrain];

            if (initWorldGenHandlers != null) { //This should remove GenTerra's registry of it's calls. Will have to see how/if this will cause mod compatability issues. Likely will not work with other mods that need to modify GenTerra, or otherwise access it's variables.
                List<int> toRemove = new List<int>();

                for (int i = 0; i < initWorldGenHandlers.Count; i++) {
                    var type = initWorldGenHandlers[i].Method.DeclaringType;
                    if (type == typeof(GenTerra)) {
                        toRemove.Add(i);
                    }
                }
                for (int i = toRemove.Count - 1; i >= 0; i--) {
                    initWorldGenHandlers.RemoveAt(toRemove[i]);
                }
            }
            if (onChunkColumnGenHandlers != null) {
                List<int> toRemove = new List<int>();

                for (int i = 0; i < onChunkColumnGenHandlers.Count; i++) {
                    var type = onChunkColumnGenHandlers[i].Method.DeclaringType;
                    if (type == typeof(GenTerra)) {
                        toRemove.Add(i);
                    }
                }
                for (int i = toRemove.Count - 1; i >= 0; i--) {
                    onChunkColumnGenHandlers.RemoveAt(toRemove[i]);
                }
            }
        ...
        public void InitWorldGen() {
            ...
            noiseScale = Math.Max(1, (api.WorldManager.MapSizeY - 64) / 256f);
            ...
            tempDataThreadLocal = new ThreadLocal<ThreadLocalTempData>(() => new ThreadLocalTempData {
                ...
                landformWeights = new float[LandformHeightNoise.landforms.LandFormsByIndex.Length]
            });
        ...
        public void InitGenTerraPretyLandforms() {
            landforms = LandformHeightNoise.landforms;
        ...
        private void generate(IServerChunk[] chunks, int chunkX, int chunkZ, bool requiresChunkBorderSmoothing) {
            ...
            Parallel.For(0, chunksize * chunksize, new ParallelOptions() { MaxDegreeOfParallelism = maxThreads }, chunkIndex2d => {
                ...
                float distY = oceanicity + upHeavalStrength;//ComputeOceanAndUpheavalDistY(upHeavalStrength, worldX, worldZ, distGeo);
                ...
                for (int posY = 1; posY <= mapsizeYm2; posY++) {
                    ...
                    StartSampleDisplacedYThreshold(posY + distY, mapsizeYm2 - 64, out int distortedPosYBase);
        ...
        public LerpedWeightedIndex2DMap GetOrLoadLerpedLandformMapFromRegion(IMapRegion mapregion, int regionX, int regionZ) {
            // 1. Load?
            LandformMapByRegion.TryGetValue(regionZ * regionMapSize + regionX, out LerpedWeightedIndex2DMap map);
            if (map != null) return map;
